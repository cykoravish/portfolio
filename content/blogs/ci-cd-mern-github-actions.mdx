...
For the longest time, I deployed my [MERN applications](/projects) by uploading files through FTP. Yeah, in 2024. I'm not proud of it.
...
Those YAML configuration files looked terrifying. All the indentation, the nested structures, the syntax that would break if you added one extra space. It felt like something only [DevOps engineers](/services) with years of experience could understand.
...
I'm not using Vercel or Netlify. I've got a VPS where I run my own [Docker containers](/services). So the workflow needs to SSH into my server, pull the latest code, rebuild containers, and restart everything.
...
Since most of my stack is built with [Next.js and Node.js](/skills-tools), keeping versions aligned between local and CI environments became critical.
