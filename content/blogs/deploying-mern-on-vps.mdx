...
But I wanted to know what was actually happening under the hood. So I rented a VPS and decided to deploy my [MERN application](/projects) the old-fashioned way.
...
Next step: get [Node.js and MongoDB](/skills-tools), Nginx, and PM2 installed. Each one has its own multi-step process.
...
Eventually I wrote a deploy script that GitHub Actions runs whenever I push to main as part of my broader [DevOps deployment workflow](/services). The workflow SSHs into the server and runs all the commands automatically. I wrote more about that process in my guide on [setting up CI/CD for MERN with GitHub Actions](/blog/ci-cd-mern-github-actions). Tests pass, code deploys. No manual work.
