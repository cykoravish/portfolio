...
My Node.js API inside one of my [MERN stack projects](/projects) was slow. Not unusably slow, but slow enough that users noticed. The dashboard took 3 seconds to load. That's forever in web time.
...
I had no indexes on my frequently queried fields. Since I use [MongoDB with Node.js](/skills-tools) for most backend work, missing indexes became a real bottleneck.
...
I used Redis at first as part of my broader [DevOps optimization setup](/services). Then realized for my scale, an in-memory cache was enough. Node-cache did the job.
...
- Adding more RAM to the server. The bottleneck was I/O, not memory. I realized this the hard way while deploying on a VPS, which I wrote about in my post on [deploying MERN on a VPS](/blog/deploying-mern-on-vps).
